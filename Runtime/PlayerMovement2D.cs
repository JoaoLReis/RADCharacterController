...
namespace RADCharacterController
{
    public class PlayerMovement2D : MonoBehaviour
    {
        ...
        private void MoveVertically(bool didJump, float intensityVertical)
        {
            ...
            else if (!isDropping && intensityVertical < -EPSILON &&
                     currentPlatform.gameObject.layer == LayerMask.NameToLayer("Floor"))
            {
                isAirborne = true;
        ...
        private void CheckIfFloored()
        {
            ...
            var hit = Physics2D.Raycast(transform.position, -transform.up, 1 + FLOAT_DISTANCE,
                (1 << LayerMask.NameToLayer("Floor")) | (1 << LayerMask.NameToLayer("Walls")));
            if (hit.collider != null && hit.distance > 0.5f)
            {
                currentPlatform = hit.collider.gameObject;
                isAirborne = false;
                transform.position = new Vector2(transform.position.x,
                    transform.position.y + (1 + FLOAT_DISTANCE) - hit.distance);
            }
        ...
        private void CheckWallCollisions()
        {
            ...
            hit = Physics2D.Raycast(transform.position, transform.right, 1 + FLOAT_DISTANCE,
                1 << LayerMask.NameToLayer("Walls"));
            if (hit.collider != null) frameMovement.x = Mathf.Min(frameMovement.x, 0);

            Debug.DrawRay(transform.position, -transform.right * (1 + FLOAT_DISTANCE), Color.yellow, 1);
            hit = Physics2D.Raycast(transform.position, -transform.right, 1 + FLOAT_DISTANCE,
                1 << LayerMask.NameToLayer("Walls"));
            if (hit.collider != null) frameMovement.x = Mathf.Max(frameMovement.x, 0);
        }
    }
}